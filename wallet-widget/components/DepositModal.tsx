...
export default function DepositModal({ isOpen, onClose, userId, apiUrl }: DepositModalProps) {
    ...
    const boostPriority = async () => {
        try {
            // Fire and forget call to boost priority
            await fetch(`${apiUrl}/v1/wallets/${userId}/priority`, {
                method: 'POST',
            });
            console.log('🚀 Priority boosted for user:', userId);
        } catch (error) {
    ...
    const fetchWallets = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`${apiUrl}/v1/wallets/${userId}`);
            ...
            // Transform wallet-server response to widget format
            const transformedWallets: Wallet[] = [];
            ...
            if (data.wallets?.solana) {
                transformedWallets.push({
                    ...
                    chainName: data.wallets.solana.chain,
                });
            }
            ...
            if (data.wallets?.base) {
                transformedWallets.push({
                    ...
                    chainName: data.wallets.base.chain,
                });
            }
            ...
            if (data.wallets?.arbitrum) {
                transformedWallets.push({
                    ...
                    chainName: data.wallets.arbitrum.chain,
                });
            }

            setWallets(transformedWallets);
            if (transformedWallets.length > 0) {
                setSelectedWallet(transformedWallets[0]);
            }
        } catch (error) {
            console.error('Error fetching wallets:', error);
            setError('Unable to load wallets. Please try again later.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="relative w-full max-w-md rounded-2xl bg-gray-800 p-6 shadow-2xl">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">by LYNX Pay</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {loading ? (
                    ...
                    <div className="rounded-xl bg-red-900/20 border border-red-500/50 p-4 text-center text-red-400">
                        {error}
                    </div>
                ) : (
